...
It has been further extended to support advanced ML vs. Classical comparisons.
"""
    ...
    for i in range(365):
        ...
        for site in sites:
            if "Eindhoven" in site:
                base_yields = [0.995, 0.97, 0.95, 0.98]
            elif "Shanghai" in site:
                base_yields = [0.99, 0.98, 0.96, 0.97]
            else:
                base_yields = [0.99, 0.92, 0.95, 0.96]
...
def _generate_statistical_tool_data(base_date: date) -> Dict[str, Any]:
    ...
    ht_data = pd.DataFrame({'before_change': np.random.normal(25.5, 1.2, 50), 'after_change': np.random.normal(26.8, 1.1, 50), 'supplier_a': np.random.normal(50.1, 0.5, 50), 'supplier_b': np.random.normal(50.6, 0.5, 50), 'supplier_c': np.random.normal(50.1, 0.5, 50)})
    process_data_df = pd.DataFrame({'timestamp': pd.to_datetime(pd.date_range(start=base_date, periods=200, freq='D')), 'seal_strength': np.concatenate([np.random.normal(88, 2.0, 100), np.random.normal(82, 2.8, 100)])})
    process_specs = {"seal_strength": {"lsl": 78, "usl": 92, "target": 85}}
    return {"gage_rr_data": pd.DataFrame(gage_data), "doe_data": doe_df, "hypothesis_testing_data": ht_data, "process_data": process_data_df, "process_specs": process_specs}
...
def _create_mbb_model(version: int) -> Dict[str, Any]:
    ...
    model_data = {"data_version": version, "yield_data": yield_data, "copq_data": copq_data, "dmaic_projects": dmaic_projects, "global_kpis": rty_by_date, "optimization_data": optimization_data, "failure_clustering_data": clustering_data, "kaizen_events": kaizen_events, "training_materials": training_materials}
...
class SessionStateManager:
    ...
    _CURRENT_DATA_VERSION = 9.0 # SME Expanded ML Lab Version
