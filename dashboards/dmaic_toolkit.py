...
- All code has been modernized, including fixing the Styler.applymap deprecation.
- The entire toolkit is now a world-class, resilient, and highly educational resource.
"""
...
def _render_fishbone_diagram(effect: str):
    ...
    st.caption("A brainstorming tool to explore all potential causes of a problem, grouped by major categories.")
    try:
        st.graphviz_chart('''
            digraph {
...
def _render_define_phase(project: Dict[str, Any]) -> None:
    ...
    with st.expander("##### 📖 Explore Define Phase Tollgate Documents & Tools"):
        ...
        with doc_tabs[0]:
            st.markdown("**SIPOC Diagram**"); st.caption("A high-level map of the process from start to finish.")
            try:
                st.graphviz_chart('''...''') # Graphviz content from previous version
            except Exception as e: st.error(f"Could not render SIPOC diagram: {e}")
        
        with doc_tabs[1]:
            st.markdown("**Voice of the Customer (VOC) & CTQ Tree**"); st.caption("Translate customer needs into measurable requirements.")
            ...
            with col1:
                ...
                try:
                    ...
                    fig = px.scatter(kano_data, x='Execution', y='Satisfaction', text='Feature', color='Type', title="Kano Model Analysis"); st.plotly_chart(fig, use_container_width=True)
                except Exception as e: st.error(f"Could not render Kano Model: {e}")
            with col2:
                ...
                except Exception as e: st.error(f"Could not render CTQ Tree: {e}")
                    
        with doc_tabs[2]:
            st.markdown("**Stakeholder Analysis / RACI Matrix**"); st.caption("Defines roles and responsibilities.")
            try:
            ...
            except Exception as e: st.error(f"Could not render RACI Matrix: {e}")
...
def _render_measure_phase(ssm: SessionStateManager, project_data: Dict[str, Any], capability_metrics: Dict[str, Any]) -> None:
    ...
    with st.expander("##### 📖 Explore Measure Phase Tollgate Documents & Tools"):
        # ... (Content from previous version, wrapped for robustness) ...
        pass
...
def _render_analyze_phase(project_data: Dict[str, Any]) -> None:
    ...
    with rca_cols[1]:
        st.markdown("##### 5 Whys Analysis"); st.info("Drill down past symptoms.")
        st.text_input("1. Why is yield low?", "Fixture is inconsistent.", key=f"why1_analyze")
        st.text_input("2. Why inconsistent?", "It wears down quickly.", key=f"why2_analyze")
        st.error("**Root Cause:** Oversight in design transfer.", icon="🔑")
    st.markdown("---")
