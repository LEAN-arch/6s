...
- The complex and buggy validation function has been removed in favor of clear,
  explicit, and in-scope data checks.
...
def _render_measure_phase(ssm: SessionStateManager, project_data: Dict[str, Any], capability_metrics: Dict[str, Any]) -> None:
    ...
    if baseline_series.empty or not specs:
        st.warning("Baseline data or specifications are missing for this project.")
        return
    ...
    try:
        ...
        st.error(f"Could not render baseline charts: {e}")
    ...
    try:
        ...
        st.error(f"Could not perform Gage R&R analysis: {e}")
...
def _render_analyze_phase(project_data: Dict[str, Any]) -> None:
    ...
    if ht_shifts is None or ht_shifts.empty:
        st.warning("Hypothesis testing data not available for this project.")
    else:
        try:
    ...
    with st.expander("##### 📖 Explore Analyze Phase Tollgate Documents & Tools"):
        ...
        with doc_tabs[0]:
            ...
            try:
                ...
                st.error(f"Could not render Pareto chart: {e}")
        with doc_tabs[1]:
            ...
            try:
                ...
                st.error(f"Could not render FMEA chart: {e}")
        with doc_tabs[2]:
            ...
            try:
                ...
                model = sm.OLS(y, sm.add_constant(X)).fit(); st.code(f"{model.summary()}")
            except Exception as e:
                st.error(f"Could not perform regression analysis: {e}")

# ==============================================================================
# IMPROVE & CONTROL PHASE HELPERS
# ==============================================================================
...
def _render_control_phase(project_data: Dict[str, Any], capability_metrics: Dict[str, Any]) -> None:
    ...
    specs = project_data.get("specs", {})
    metric_name = project_data.get("metric_name", "Measurement")
    if not specs or not capability_metrics:
        st.warning("Cannot generate control phase charts without spec and baseline capability data.")
        return
    ...
    try:
        ...
        st.error(f"Could not render control charts: {e}")
