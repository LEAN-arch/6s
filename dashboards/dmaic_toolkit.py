...
- Implemented "Graceful Degradation": each individual tool/plot is wrapped
...
def _render_measure_phase(ssm: SessionStateManager, project_data: Dict[str, Any], capability_metrics: Dict[str, Any]) -> None:
    ...
    with st.expander("##### 📖 Explore Measure Phase Tollgate Documents & Tools"):
        ...
        with doc_tabs[0]:
            ...
            st.dataframe({'Metric to Collect': [metric_name], 'Data Type': ['Continuous'], 'Tool': ['Calipers #DC-04'], 'Sample Size': ['5 units/hr']}, hide_index=True)
        with doc_tabs[1]:
