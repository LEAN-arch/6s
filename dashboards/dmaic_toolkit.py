...
def render_fishbone_diagram(effect: str):
    """Renders a dynamic Fishbone (Ishikawa) diagram."""
    ...
    for i, col in enumerate(cols):
        with col:
            ...
            for cause in causes[categories[i]]: st.markdown(f"- {cause}")
    ...
    try:
        ...
        with phase_tabs[0]:
            ...
            with st.container(border=True):
                ...
                st.markdown("---"); st.info(f"**Problem Statement:**\n\n> {project['problem_statement']}"); st.success(f"**Goal Statement (S.M.A.R.T.):**\n\n> {project['goal_statement']}")
            with st.expander("View Define Phase Tools"):
                st.markdown("#### SIPOC Diagram"); st.info("A high-level map of the process from Supplier to Customer.")
                sipoc_data = {"Suppliers": ["Component Vendors", "Sub-Assembly Line"], "Inputs": ["Capacitors", "PCBs", "Housing"], "Process": ["Inspect -> Assemble -> Solder -> Test"], "Outputs": ["Functional Charging Module"], "Customers": ["Main Assembly Line", "Final Product"]}
        ...
        with phase_tabs[1]:
            ...
            process_df, specs = ssm.get_data("process_data"), ssm.get_data("process_specs")["seal_strength"]
            ...
            cap_cols[0].metric("Cpk (Baseline)", f"{cpk:.2f}", "Target: > 1.33"); cap_cols[1].metric("Ppk (Baseline)", f"{ppk:.2f}", "Target: > 1.33"); cap_cols[2].metric("DPMO (Estimated)", f"{project['baseline_dpmo']:,}"); cap_cols[3].metric("Sigma Level (Baseline)", f"{1.5 + norm.ppf(1 - project['baseline_dpmo'] / 1_000_000):.2f} σ")
            ...
            st.markdown("---"); st.markdown("##### 2. Measurement System Analysis (Gage R&R)")
        ...
        with phase_tabs[2]:
            ...
            with st.expander("Learn More: Common RCA Tools"):
                st.markdown("- **5 Whys:** A simple, iterative technique to drill down past symptoms to find the true root cause.\n- **Fishbone Diagram:** A structured brainstorming tool to organize potential causes into categories.")
            rca_cols = st.columns(2)
            with rca_cols[0]:
                st.info("**5 Whys Analysis**"); st.text_input("1. Why is yield low?", "The alignment fixture is inconsistent.", key=f"why1_{project['id']}"); st.text_input("2. Why is it inconsistent?", "It wears down quickly.", key=f"why2_{project['id']}"); st.text_input("3. Why does it wear down?", "The material hardness spec is too low.", key=f"why3_{project['id']}"); st.text_input("4. Why is the spec low?", "It was based on an older, lower-throughput model.", key=f"why4_{project['id']}"); st.text_input("5. Why was it not updated?", "Process oversight during design transfer.", key=f"why5_{project['id']}")
            with rca_cols[1]:
                render_fishbone_diagram(effect=project['problem_statement'])
            st.markdown("---"); st.markdown("##### Hypothesis Testing Suite")
            ht_data, ht_tool = ssm.get_data("hypothesis_testing_data"), st.selectbox("Select Hypothesis Test:", ["2-Sample t-Test (Before vs. After)", "ANOVA (Supplier A vs. B vs. C)"])
            if ht_tool == "2-Sample t-Test (Before vs. After)":
                fig, result = perform_t_test(ht_data['before_change'], ht_data['after_change'], "Before Change", "After Change"); st.plotly_chart(fig, use_container_width=True)
                ...
                fig, result = perform_anova(df_anova, 'value', 'group', "Component Strength by Supplier"); st.plotly_chart(fig, use_container_width=True)
        ...
        with phase_tabs[3]:
            ...
            doe_data, factors, response = ssm.get_data("doe_data"), ['temp', 'time', 'pressure'], 'strength'
            ...
            with st.expander("Learn More: Interpreting DOE Plots"):
                st.markdown("- **Main Effects Plot:** Shows the average impact each factor has on the response.\n- **Interaction Plot:** Visualizes how one factor's effect changes based on another's level. **Non-parallel lines indicate interaction.**\n- **Response Surface:** A 3D map of the predicted response, visualizing the optimal process window.")
            main_effects_fig, interaction_fig, surface_fig = create_doe_plots(doe_data, factors, response)
            st.plotly_chart(main_effects_fig, use_container_width=True); st.plotly_chart(interaction_fig, use_container_width=True); st.plotly_chart(surface_fig, use_container_width=True)
        ...
        with phase_tabs[4]:
            ...
            controlled_process = pd.Series(np.random.normal(loc=88, scale=0.8, size=100))
            spc_fig = create_control_chart(controlled_process, "Bond Strength", 80, 90)
